...
MERGE INTO WALMART_EDW.store_dim t1
...
MERGE INTO WALMART_EDW.PRODUCT_DIM t1
...
----- # 3 calendar dim
TRUNCATE TABLE WALMART_EDW.calendar_dim;
INSERT INTO WALMART_EDW.calendar_dim
(	
	cal_dt,
	cal_type_name,
	day_of_wk_num,
...
SET LAST_DATE = (SELECT MAX(cal_dt) FROM WALMART_EDW.sales_inv_store_dy);
...
-----------we need to delete the original latest date in the fact table WALMART_EDW.sales_inv_store_dy and append the new records from that date.
DELETE FROM WALMART_EDW.sales_inv_store_dy WHERE cal_dt=$LAST_DATE;

CREATE OR REPLACE TRANSIENT TABLE WALMART_EDW.daily_sales_stg AS 
SELECT 
	trans_dt AS cal_dt,
...
CREATE OR REPLACE TRANSIENT TABLE WALMART_EDW.daily_inventory_stg AS 
SELECT 
cal_dt   AS cal_dt,
store_key   AS store_key,
prod_key   AS prod_key,
...
INSERT INTO WALMART_EDW.sales_inv_store_dy 
(
	cal_dt,
	store_key,
	prod_key,
...
FROM WALMART_EDW.daily_sales_stg s
FULL OUTER JOIN WALMART_EDW.daily_inventory_stg i USING (cal_dt, store_key, prod_key)
;
SELECT * FROM WALMART_EDW.sales_inv_store_dy;

TRUNCATE TABLE IF EXISTS WALMART_EDW.sales_inv_store_wk;
INSERT INTO WALMART_EDW.sales_inv_store_wk
(
	yr_num,
	wk_num,
	store_key,
...
FROM WALMART_EDW.sales_inv_store_dy s
JOIN WALMART_EDW.calendar_dim c USING (cal_dt)
GROUP BY 1,2,3,4
ORDER BY 1,2,3,4;
